...
const fileUrls = {
  "prokat.xlsx":
    "https://cdn.glitch.global/f2d6eaf0-a2fc-47db-95ff-ebbbdd6244e2/prokat.xlsx?v=1720987295117",
  "umka.xlsx":
    "https://cdn.glitch.global/f2d6eaf0-a2fc-47db-95ff-ebbbdd6244e2/umka.xlsx?v=1720987292298",
  "svatik.xlsx":
    "https://cdn.glitch.global/f2d6eaf0-a2fc-47db-95ff-ebbbdd6244e2/svatik.xlsx?v=1720987285091",
  "start_1.png":
    "https://cdn.glitch.global/f2d6eaf0-a2fc-47db-95ff-ebbbdd6244e2/start_1.png?v=1720987298877",
  "nameSearch.png":
    "https://cdn.glitch.global/f2d6eaf0-a2fc-47db-95ff-ebbbdd6244e2/nameSearch.png?v=1720987305586",
};

// Функція для завантаження файлів з URL і збереження їх локально
...
const loadAllExcelFiles = async () => {
  await Promise.all(
    Object.keys(fileUrls).map((fileName) => {
      const filePath = path.join(__dirname, "assets", fileName);
      return downloadFile(fileUrls[fileName], filePath);
    })
  );

  const fileNames = Object.keys(fileUrls);
  return fileNames.reduce((acc, fileName) => {
    return acc.concat(loadExcelFile(fileName));
  }, []);
};
...
(async () => {
  rows = await loadAllExcelFiles();
})();

const startGame = async (chatId) => {
  await bot.sendMessage(
    chatId,
    "Зараз я загадаю число від 0 до 9 а ти відгадуй"
  );
...
const start = () => {
  ...
  bot.on("message", async (msg) => {
    ...
    if (text === "/start") {
      await bot.sendMessage(
        chatId,
        `Щоб почати користуватися ботом треба тикнути цю кнопочку`
      );
      await bot.sendPhoto(
        chatId,
        path.join(__dirname, "assets", "start_1.png")
      );
      await bot.sendMessage(chatId, `ДУМАЙТЕ ЩО ПИШИТЕ В ПОШУК ПО НАЗВІ`);
      await bot.sendMessage(
        chatId,
        `Не пишіть просто "штани" чи щось бо буде дуже спамити повідомленями таке краще вписуйте модель!!!!`
      );
      await bot.sendPhoto(
        chatId,
        path.join(__dirname, "assets", "nameSearch.png")
      );
      await bot.sendMessage(
        chatId,
        `Назви складів umka - Умка, svatik - святік, prokat - прокат`
      );
      await bot.sendMessage(chatId, `Вітаю вас в базі даних UMKA.SHOP`);
      return bot.sendSticker(
        chatId,
        "https://sl.combot.org/tommood_by_moe_sticker_bot/webp/33xf09f9299.webp"
      );
    }
    ...
    if (chatState[chatId] === "awaiting_barcode") {
      ...
      const result = rows.find(
        (row) => String(row["Штрих-код"]) === String(barcode)
      );
    ...
    if (chatState[chatId] === "awaiting_article") {
      ...
      const results = rows.filter(
        (row) => String(row["Артикул"]) === String(article)
      );
    ...
    if (chatState[chatId] === "awaiting_name") {
      ...
      const results = rows.filter((row) =>
        String(row["Назва товару"]).toLowerCase().includes(name)
      );
  ...
  bot.on("callback_query", async (msg) => {
    ...
    if (Number(data) === chats[chatId]) {
      ...
      return bot.sendSticker(
        chatId,
        "https://sl.combot.org/peachngomavnt/webp/55xf09f8e89.webp"
      );
    } else {
      return await bot.sendMessage(
        chatId,
        `Нажаль ти не вгадав цифру, бот загадав цифру: ${chats[chatId]}`,
        againOptions
      );
    }
  });
